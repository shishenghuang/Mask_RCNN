...
for idx, file_name in enumerate(file_names):
    ...
    exported_class_ids = [class_conversion[t] for t in exported_class_ids]

    end_time = time.time()
    total_time += (end_time - start_t)
    total_pictures += 1
    print("Current FPS @", total_pictures / total_time, "Hz, Current = ", 1 / (end_time - start_t))
    # save_id_image(id_image, OUTPUT_DIR, base_name, exported_class_ids, STORE_CLASS_IDS, exported_rois)
    # print(id_image.dtype, id_image.shape)
    print(exported_class_ids)
    print(np.max(id_image), np.min(id_image))
